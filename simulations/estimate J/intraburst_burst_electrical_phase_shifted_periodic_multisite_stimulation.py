...
phase_shifts = []
for kPhaseShift in range(nElectrodes-1):
	phase_shifts.append(  float(sys.argv[10+kPhaseShift]) )
...
filter_intra_population_weights = np.zeros( (N,N) )
filter_intra_population_weights[ 0 : NstimNeurons , 0 : NstimNeurons ]=np.ones( (NstimNeurons,NstimNeurons) )
filter_intra_population_weights[ NstimNeurons : 2*NstimNeurons , NstimNeurons : 2*NstimNeurons ]=np.ones( (NstimNeurons,NstimNeurons) )

filter_inter_population_weights_1n2 = np.zeros( (N,N) )
filter_inter_population_weights_1n2[NstimNeurons:2*NstimNeurons,:NstimNeurons] = np.ones( (NstimNeurons,NstimNeurons) )

filter_inter_population_weights_2n1 = np.zeros( (N,N) )
filter_inter_population_weights_2n1[:NstimNeurons,NstimNeurons:2*NstimNeurons] = np.ones( (NstimNeurons,NstimNeurons) )

# import matplotlib.pyplot as plt
# plt.imshow(filter_inter_population_weights)
# plt.show()
# exit()

cRsignal=functions_stim.intra_burst_frequency_multiple_spikes_phase_shifted_periodic_multisite_stimulation(fCR = fCR, totalStimulationTime = Tstim, M = nElectrodes, dt = dt, phase_shifts = phase_shifts, e_pulse_scale = e_pulse_scale, pulses_per_burst = numberOfPulsesPerBurst, intraburst_frequency = f_intra_burst )
cRsignal.initialize_intra_burst_frequency_multiple_spikes_phase_shifted_periodic_multisite_stimulation( )
...
meanWeightTimeSeries[0,:]=np.array( [0, np.mean(cMatrix[:N_STN,:N_STN]), np.mean(  np.multiply( cMatrix.A , filter_intra_population_weights ) ) , np.mean( np.multiply( cMatrix.A , filter_inter_population_weights_1n2 ) ) , np.mean( np.multiply( cMatrix.A , filter_inter_population_weights_2n1 ) ) ] )
...
for kStep in range( kStepInit, nSteps ):
	...
	if kStep % kWeightOutput == 0:	
		# first column is time step, second column mean recurrent STN weight, third column mean recurrent GPe weigth, fourth column mean STN -> GPe weight, and fifth column mean GPe -> STN weight
		meanWeightTimeSeries[int( float(kStep % kSave)/float(kWeightOutput)),:]=np.array( [kStep, np.mean(cMatrix[:N_STN,:N_STN]), np.mean(  np.multiply( cMatrix.A , filter_intra_population_weights ) ) , np.mean( np.multiply( cMatrix.A , filter_inter_population_weights_1n2 ) ) , np.mean( np.multiply( cMatrix.A , filter_inter_population_weights_2n1 ) ) ] )
