...
for kPhaseShift in range(nElectrodes-1):
	phase_shifts.append(  float(sys.argv[9+kPhaseShift]) )
...
wSignalArray[0:NstimNeurons,0] = 1.0

cRsignal=functions_stim.multiple_spikes_phase_shifted_periodic_multisite_stimulation(fCR = fCR, totalStimulationTime = Tstim, M = nElectrodes, dt = dt, phase_shifts = phase_shifts, e_pulse_scale = e_pulse_scale, pulses_per_burst = numberOfPulsesPerBurst )
cRsignal.initialize_multiple_spikes_phase_shifted_periodic_multisite_stimulation( )
